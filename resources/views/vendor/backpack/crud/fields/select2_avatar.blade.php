...
<style>
    ...
    <?php
        for ($i=1; $i<=count($options); $i++){
            ...
            ?>
                li[id*="{{$image}}"]{
                    background-image: url({{$options[$i-1]["avatar"]}})  ;
                    background-repeat: no-repeat;
                    background-size: 50px 50px;
                    padding-left: 70px;background-origin: padding-box;
                }
            <?php
        }
    ?>
...
<div @include('crud::inc.field_wrapper_attributes')
    data-init-function="init"
>
    <input type="hidden" id="number" value="{{count($options)}}">
    ...
    >
        @if ($entity_model::isColumnNullable($field['name']))
            <option value="" style="background-image: url(
                {{url('/uploads/141711.jpg')}}
            );">
                -
            </option>
        @endif
        ...
        @if (count($options))
            @foreach ($options as $option)
                @if($current_value == $option->getKey())
                    <option value="{{ $option->getKey() }}" selected>
                        {{ $option->{$field['attribute']} }}
                    </option>
                @else
                    <option value="{{ $option->getKey() }}">
                        <span>{{ $option->{$field['attribute']} }}</span>
                    </option>
                @endif
            @endforeach
        @endif
