...
<div class="form-group col-sm-12 checklist_dependency"
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="hidden_fields_primary" data-name = "{{ $type['name'] }}">
                @if(isset($field['value']))
                    @if(old($type['name']))
                        @foreach( old($type['name']) as $item )
                            <input type="hidden" class="primary_hidden" name="{{ $type['name'] }}[]" value="{{ $item }}">
                        @endforeach
                    @else
                    ...
                    @endif
                @else
                    <input type="hidden" class="primary_hidden" name="{{ $type['name'] }}" id="input-type">
                @endif
            </div>
            <?php $k = 0; ?>
            @foreach ($type['options'] as $connected_entity_entry)
                <div class="col-sm-{{ isset($type['number_columns']) ? intval(12/$type['number_columns']) : '4'}}">
                    <div class="checkbox">
                        <label class="font-weight-normal">
                            ...
                            @if( ( isset($field['value']) && is_array($field['value']) && in_array($connected_entity_entry->id, $field['value'][0]->pluck('id', 'id')->toArray())) || ( old($type["name"]) && in_array($connected_entity_entry->id, old( $type["name"])) ) )
                                checked = "checked"
                            @endif >
        ...
        <div class="row">
            <div class="hidden_fields_secondary" data-name="{{ $content['name'] }}">
                @if(isset($field['value']))
                    @if(old($content['name']))
                        @foreach( old($content['name']) as $item )
                            <input type="hidden" class="secondary_hidden" name="{{ $content['name'] }}[]" value="{{ $item }}">
                        @endforeach
                    @else
...
@php
    $multiple = array_get($images, 'multiple', true);
    $sortable = array_get($images, 'sortable', false);
    $value = old(square_brackets_to_dots($images['name'])) ?? $images['value'] ?? $images['default'] ?? '';

    if (!$multiple && is_array($value)) {
        $value = array_first($value);
    }

    if (!isset($images['wrapperAttributes']) || !isset($images['wrapperAttributes']['data-init-function']))
    {
        $images['wrapperAttributes']['data-init-function'] = 'bpFieldInitBrowseMultipleElement';

        if ($multiple) {
            $images['wrapperAttributes']['data-popup-title'] = trans('backpack::crud.select_files');
            $images['wrapperAttributes']['data-multiple'] = "true";
        } else {
            $images['wrapperAttributes']['data-popup-title'] = trans('backpack::crud.select_file');
            $images['wrapperAttributes']['data-multiple'] = "false";
        }

        if ($mimes = array_get($images, 'mime_types')) {
            $images['wrapperAttributes']['data-only-mimes'] = json_encode($mimes);
        }

        if($sortable){
            $images['wrapperAttributes']['sortable'] = "true";
        }
    }
@endphp
...
<div @include('crud::inc.field_wrapper_attributes') id="content1">
            ...
            @foreach( (array)$value as $v)
                @if ($v)
                    <div class="input-group input-group-sm">
                        <input type="text" name="{{ $images['name'] }}[]" value="{{ $v }}" data-marker="multipleBrowseInput"
    ...
    @else
        <input type="text" name="{{ $images['name'] }}" value="{{ $value }}" @include('crud::inc.field_attributes') readonly>
    @endif
    ...
    @if (isset($images['hint']))
        <p class="help-block">{!! $images['hint'] !!}</p>
    @endif

    <script type="text/html" data-marker="browse_multiple_template">
        <div class="input-group input-group-sm">
            <input type="text" name="{{ $images['name'] }}[]" @include('crud::inc.field_attributes') readonly>
            <div class="input-group-btn">
                <button type="button" class="browse remove btn btn-sm btn-light">
                    <i class="fa fa-trash"></i>
                </button>
...
<div @include('crud::inc.field_wrapper_attributes')  id="content2">
    ...
    <div class="controls">
	    <div class="input-group">
			<input
				type="text"
				id="{{ $image['name'] }}-filemanager"
				name="{{ $image['name'] }}"
		        value="{{ old(square_brackets_to_dots($image['name'])) ?? $image['value'] ?? $image['default'] ?? '' }}"
		        data-init-function="bpFieldInitBrowseElement"
		        data-elfinder-trigger-url="{{ url(config('elfinder.route.prefix').'/popup') }}"
		        @include('crud::inc.field_attributes')

				@if(!isset($image['readonly']) || $image['readonly']) readonly @endif
			>

			<span class="input-group-append">
			  	<button type="button" data-inputid="{{ $image['name'] }}-filemanager" class="btn btn-light btn-sm popup_selector"><i class="fa fa-cloud-upload"></i> {{ trans('backpack::crud.browse_uploads') }}</button>
				<button type="button" data-inputid="{{ $image['name'] }}-filemanager" class="btn btn-light btn-sm clear_elfinder_picker"><i class="fa fa-eraser"></i> {{ trans('backpack::crud.clear') }}</button>
			</span>
		</div>
	</div>

	@if (isset($image['hint']))
        <p class="help-block">{!! $image['hint'] !!}</p>
    @endif
...
<div data-video data-init-function="bpFieldInitVideoElement" @include('crud::inc.field_wrapper_attributes') id="content0" >
    @include('crud::inc.field_translatable_icon')
    <label>Video</label>
    <input class="video-json" type="hidden" name="{{ $video['name'] }}" value="{{ $value }}">
    <div class="input-group">
        <input @include('crud::inc.field_attributes', ['default_class' => 'video-link form-control']) type="url" id="{{ $video['name'] }}_link">
    ...
    {{-- HINT --}}
    @if (isset($video['hint']))
        <p class="help-block">{!! $video['hint'] !!}</p>
    @endif
</div>
...
<div @include('crud::inc.field_wrapper_attributes') id = "content3">
    ...
    @include('crud::inc.field_translatable_icon')
    <textarea
        id="ckeditor-{{ $content['name'] }}"
        name="{{ $content['name'] }}"
        data-init-function="bpFieldInitCKEditorElement"
        @include('crud::inc.field_attributes', ['default_class' => 'form-control'])
    	>{{ old(square_brackets_to_dots($content['name'])) ?? $content['value'] ?? $content['default'] ?? '' }}</textarea>

    {{-- HINT --}}
    @if (isset($content['hint']))
        <p class="help-block">{!! $content['hint'] !!}</p>
    @endif
</div>
